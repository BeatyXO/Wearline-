...
export function AppLayout({path,children}:{path:string;children:ReactNode}){
  const {wallet,agreementId,connect,disconnect,switchNetwork,wrongNetwork,error,notice,txHash,explorerTx,explorerAddress}=useWearline()
  ...
  useEffect(()=>{
    ...
    const outside=(event:MouseEvent)=>{if(walletMenuRef.current&&!walletMenuRef.current.contains(event.target as Node))setWalletOpen(false)}
    const escape=(event:KeyboardEvent)=>{if(event.key==='Escape')setWalletOpen(false)}
    document.addEventListener('mousedown',outside)
    document.addEventListener('keydown',escape)
    return ()=>{document.removeEventListener('mousedown',outside);document.removeEventListener('keydown',escape)}
  },[walletOpen])

  useEffect(()=>{if(!wallet)setWalletOpen(false)},[wallet])
  ...
  return <div className="app-shell">
    <header className="topbar">
      ...
      <div className="wallet-control" ref={walletMenuRef}>
        <button className={`wallet-button ${wrongNetwork?'wrong-network':''}`} aria-haspopup={wallet?'menu':undefined} aria-expanded={wallet?walletOpen:undefined} onClick={()=>wallet?setWalletOpen(v=>!v):void connect()}><Wallet size={17}/>{wallet?shortAddress(wallet):'Connect wallet'}{wallet&&wrongNetwork&&<span className="network-dot" aria-label="Wrong network"/>}</button>
